...
" Set SQL to match in any clojureString that starts with an obvious start-of-SQL-marker.
"	Take care not to consume the double-quotes (\zs & \ze).
"	Matches are case-sensitive (\C), because I always uppercase SQL keywords.
syntax region sqlSnippet start=/"\C\zs\v(SELECT|INSERT|UPDATE|DELETE|CREATE|DROP)/ end=/\ze"/ contains=@SQL containedin=clojureString
